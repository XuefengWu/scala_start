...
case class AnswerDetail(id: Pk[Long] = NotAssigned, title:Option[String], nodeId: Long, questionId: Long, choiceId: Long, testakerId: Long)

object AnswerDetail {
  val simple = {
      get[Pk[Long]]("answer.id") ~
      get[Option[String]]("node.title") ~
      get[Long]("node.id") ~
      get[Long]("question.id") ~
      get[Long]("choice.id") ~
      get[Long]("testaker.id") map {
      case id ~ title ~ nodeId ~ questionId ~ choiceId~ testakerId => AnswerDetail(id, title, nodeId,questionId, choiceId,testakerId)
    }
  }

  def findById(id: Long): Option[AnswerDetail] = {
    DB.withConnection {
      implicit c =>
        SQL(
          """
