...
object Node {
  ...
  def create(v: Node):Long = {
    DB.withConnection {
      implicit connection =>
        SQL("insert into node (createdAt,lastUpdateAt) values ({createdAt},{lastUpdateAt})").on(
  ...
  def findById(id: Long): Option[Node] = {
    DB.withConnection {
      implicit c =>
